<template>
  <div class="container-fluid">
    <h2 class="text-dark m-4">Horarios</h2>
        ...
        <div class="card-body">
          <div class="container-fluid">
            <div class="row col-12 m-0 p-0">
              ...
              <div class="text-left ml-3 col-sm-6 col-lg-4 col-xl-2 m-0 p-0">
                <button class="btn btn-warning" @click="users">
                  <i class="el-icon-plus"></i>Asignar usuarios
                </button>
              </div>
                ...
                <thead>
                  <tr class="text-center">
                    ...
                    <th @click="sort(4)" class="w-25">
                      ...
                      />
                      <i
                        v-show="this.searchfield == 4 && this.asc == true"
                        class="text-primary el-icon-caret-bottom"
                      />
                    </th>
...
export default {
  ...
  methods: {
    onClickEdit($idy) {
      const ediId = $idy;
      window.location.href = "/horarios/editar/" + ediId + "/";
      //this.$emit("edit", ediId);
    },
    onClickDetails($idy) {
      const ediId = $idy;
      window.location.href = "/horarios/detalle/" + ediId + "/";
      ...
      if (this.search == "") {
        ...
        this.content.forEach((value, index) => {
          if (this.foundIt(value.id, this.search)) {
            ...
            this.aux.push(value);
          } else if (this.foundIt(value.area.place, this.search)) {
            this.aux.push(value);
          }
        });
        this.max = Math.ceil(this.aux.length / this.pagesize);
    ...
    sort($op) {
      ...
      if ($op == 1) {
        ...
        if (this.asc) {
          this.aux.sort((a, b) =>
            a.area.place < b.area.place
              ? -1
              : Number(a.area.place > b.area.place)
          );
        } else {
          this.aux.sort((a, b) =>
            a.area.place > b.area.place
          ...
          axios
            ...
            .then((response) => {
              if (response.data != 1) {
                this.showErrorNotification(
                  "Error al eliminar",
                  "El horario se encuentra asignado"
                );
              } else {
                this.showSuccessNotification("Eliminar", "horario eliminado");
    ...
    users() {
      window.location.href = "/horarios/usuarios/0";
    },
  },
};
</script>
