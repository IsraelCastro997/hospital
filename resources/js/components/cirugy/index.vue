<template>
  <div class="container-fluid">
    <h2 class="text-dark m-4">Cirugías</h2>
...
export default {
  ...
  methods: {
    onClickEdit($idy) {
      const ediId = $idy;
      window.location.href = "/cirugias/editar/" + ediId + "/";
      //this.$emit("edit", ediId);
    },
    onClickDetails($idy) {
      const ediId = $idy;
      window.location.href = "/cirugias/detalle/" + ediId + "/";
    ...
    searchFunction() {
      ...
      if (this.search == "") {
        ...
        this.content.forEach((value, index) => {
          if (this.foundIt(value.id, this.search)) {
            this.aux.push(value);
          } else if (this.foundIt(value.type, this.search)) {
            this.aux.push(value);
          } else if (this.foundIt(value.patient.name, this.search)) {
            this.aux.push(value);
          } else if (this.foundIt(this.getDate(value.dateline), this.search)) {
            this.aux.push(value);
          }
        });
        this.max = Math.ceil(this.aux.length / this.pagesize);
    ...
    sort($op) {
      ...
      if ($op == 1) {
        ...
        if (this.asc) {
          this.aux.sort((a, b) =>
            this.getDateO(a.dateline) < this.getDateO(b.dateline)
              ? -1
              : Number(this.getDateO(a.dateline) > this.getDateO(b.dateline))
          );
        } else {
          this.aux.sort((a, b) =>
            this.getDateO(a.dateline) > this.getDateO(b.dateline)
    ...
    onClickDelete($idc, $iddel) {
      this.$confirm("Realmente desea eliminar la cirugía", "Alerta", {
          ...
          axios
            ...
            .then((response) => {
              if (response.data != 1) {
                this.showErrorNotification(
                  "Error al eliminar",
                  "La cirugía se encuentra asignado"
                );
              } else {
                this.showSuccessNotification("Eliminar", "Cirugía eliminada");
                var index = this.aux.findIndex((i) => i.id === $idc);
                if (index != -1) this.aux.splice(index, 1);
                axios.get("/cirugias/all").then((res) => {
                  this.content = res.data;
                });
                this.max = Math.ceil(this.aux.length / this.pagesize);
                this.update();
    ...
    add() {
      window.location.href = "/cirugias/agregar";
    },
    getDate(dat) {
      var d = new Date(dat);
      var datestring =
