  ...
  <div class="container-fluid">
    <div class="row col-12 m-0 p-0">
      <div class="text-left col-sm-3 col-lg-2 col-xl-1 m-0 p-0">
        <button class="btn btn-success col-12 m-0 pl-0 pr-0" @click="add()">
...
export default {
  ...
  mounted() {
    axios
      .get(`/pacientes/detalleone/${this.detailsid}`)
      .then((res) => {
        this.data = res.data.diagnoses;
        this.content = res.data.diagnoses;
  ...
  methods: {
    onClickEdit($idy) {
      const ediId = $idy;
      window.location.href = "/historiales/editar/" + ediId + "/";
      //this.$emit("edit", ediId);
    },
    onClickDetails($idy) {
      const ediId = $idy;
      window.location.href = "/historiales/detalle/" + ediId + "/";
    ...
    onClickDelete($idc, $iddel) {
      this.$confirm("Realmente desea eliminar el diagnóstico", "Alerta", {
