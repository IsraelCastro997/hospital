<template>
  <div class="container-fluid">
    <h2 class="text-dark m-4">Inventario</h2>
...
export default {
  ...
  methods: {
    onClickEdit($idy) {
      const ediId = $idy;
      window.location.href = "/inventario/editar/" + ediId + "/";
      //this.$emit("edit", ediId);
    },
    onClickDetails($idy) {
      const ediId = $idy;
      window.location.href = "/inventario/detalle/" + ediId + "/";
    ...
    onClickDelete($idc, $iddel) {
      this.$confirm("Realmente desea eliminar el artículo", "Alerta", {
        ...
        .then(() => {
          axios
            .delete(`/inventario/${$idc}`)
            .then((response) => {
              if (response.data != 1) {
                this.showErrorNotification(
                  "Error al eliminar",
                  "El artículo se encuentra asignado"
                );
              } else {
    ...
    add() {
      window.location.href = "/inventario/agregar";
    },
    getDate(dat) {
      var d = new Date();
      var datestring =
